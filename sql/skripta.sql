...
ALTER TABLE korisnici
    ADD COLUMN aktivan BOOLEAN NOT NULL DEFAULT TRUE;

UPDATE korisnici
SET aktivan = TRUE
WHERE aktivan IS NULL;

DROP TABLE upitnik_odgovori;

CREATE TABLE pitanja_upitnik (
     id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
     upitnik_id INT NOT NULL,
     pitanja_id INT NOT NULL,

     CONSTRAINT uq_uo_pitanja_upitnik UNIQUE (upitnik_id, pitanja_id),
...
CREATE INDEX idx_uo_upitnik ON pitanja_upitnik(upitnik_id);
CREATE INDEX idx_uo_pitanja ON pitanja_upitnik(pitanja_id);

DROP TABLE pitanja_upitnik;